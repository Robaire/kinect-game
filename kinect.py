...
class Game():
	def __init__(self, width, height, title):
		...
		# List of Projectiles
		self.projectiles = [
			Projectile(self.width, self.height, "comicsansms", "Foisie is great!", "foisie", 3),
			Projectile(self.width, self.height, "comicsansms", "Propaganda Screen", "atwater_kent", 3),
	...
	def get_state(self):
		...
		if self._bodies is not None:  # If there is a body in the frame
			for i in range(0, self._kinect.max_body_count):
				...
				x_alt = 2 * x_alt - 1
				x_alt = (abs(x_alt) ** (1. / 3)) * sign(x_alt)
				x_alt = (x_alt + 1) / 2
